  ...
  return (
    <div className="w-full space-y-6 pb-12">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {deletedProjects.map((project: any) => (
            <article key={project._id} className={`relative flex min-h-[210px] flex-col rounded-xl border bg-white ${selected.projects.includes(project._id) ? "border-blue-400 ring-2 ring-blue-100" : "border-slate-200"}`}>
              <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3"><label className="inline-flex cursor-pointer items-center gap-2 text-xs font-medium text-rose-600"><input type="checkbox" aria-label={`Chọn dự án ${project.title}`} checked={selected.projects.includes(project._id)} onChange={() => toggleSelected(project._id)} className="size-4 rounded" /><Trash2 className="size-3.5" />Dự án đã xoá</label><span className="flex items-center gap-1.5 text-[11px] text-slate-400"><CalendarClock className="size-3.5" />{project.deletedAt ? new Date(project.deletedAt).toLocaleDateString("vi-VN") : "—"}</span></div>
              <div className="min-w-0 flex-1 px-4 py-4"><h2 className="line-clamp-2 break-words text-base font-bold leading-6 text-slate-900 [overflow-wrap:anywhere]">{project.title}</h2><p className="mt-1.5 line-clamp-2 break-words text-sm leading-5 text-slate-500 [overflow-wrap:anywhere]">{project.description || "Dự án không có mô tả."}</p><p className="mt-4 flex min-w-0 items-center gap-2 text-xs text-slate-600"><Building2 className="size-3.5 shrink-0 text-slate-400" /><span className="truncate">{project.workspace?.name || "Không xác định workspace"}</span></p></div>
      ...
      {!showSkeleton && activeTab === "workspaces" && deletedWorkspaces.length > 0 && <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{deletedWorkspaces.map((workspace: any) => {
        ...
        return <article key={workspace._id} className={`relative flex min-h-[190px] min-w-0 flex-col overflow-hidden rounded-xl border bg-white ${selected.workspaces.includes(workspace._id) ? "border-blue-400 ring-2 ring-blue-100" : "border-slate-200"}`}><div className="flex items-center justify-between border-b border-slate-100 px-4 py-3"><label className="inline-flex cursor-pointer items-center gap-2 text-xs font-medium text-rose-600"><input type="checkbox" aria-label={`Chọn workspace ${workspace.name}`} checked={selected.workspaces.includes(workspace._id)} onChange={() => toggleSelected(workspace._id)} className="size-4 rounded" /><Trash2 className="size-3.5" />Workspace đã xoá</label><span className="flex items-center gap-1.5 text-[11px] text-slate-400"><CalendarClock className="size-3.5" />{workspace.deletedAt ? new Date(workspace.deletedAt).toLocaleDateString("vi-VN") : "—"}</span></div><div className="min-w-0 flex-1 px-4 py-4"><h2 className="line-clamp-2 break-words text-base font-bold leading-6 text-slate-900 [overflow-wrap:anywhere]">{workspace.name}</h2><p className="mt-1.5 line-clamp-2 break-words text-sm leading-5 text-slate-500 [overflow-wrap:anywhere]">{workspace.description || "Workspace không có mô tả."}</p><p className="mt-4 text-xs font-medium text-slate-500">{workspace.members?.filter((member: any) => member.status !== "pending").length || 1} thành viên</p></div><div className="flex items-center justify-between border-t border-slate-100 px-4 py-3"><span className={`rounded-md px-2 py-1 text-[10px] font-semibold ${daysRemaining <= 7 ? "bg-rose-50 text-rose-700" : "bg-amber-50 text-amber-700"}`}>Tự xoá sau {daysRemaining} ngày</span><Button variant="ghost" size="sm" disabled={isRestoringWorkspace} onClick={() => restoreDeletedWorkspace(workspace._id)} className="h-8 gap-1.5 px-2.5 text-blue-600 hover:bg-blue-50 hover:text-blue-700"><RotateCcw className="size-3.5" />Khôi phục</Button></div></article>;
      })}</div>}
            ...
            return (
              <article key={task._id} className={`relative flex min-h-[244px] flex-col rounded-xl border bg-white ${selected.tasks.includes(task._id) ? "border-blue-400 ring-2 ring-blue-100" : "border-slate-200"}`}>
                <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                  <label className="inline-flex cursor-pointer items-center gap-2 text-xs font-medium text-rose-600"><input type="checkbox" aria-label={`Chọn công việc ${task.title}`} checked={selected.tasks.includes(task._id)} onChange={() => toggleSelected(task._id)} className="size-4 rounded" /><Trash2 className="size-3.5" />Đã xoá</label>
                ...
                <div className="flex-1 px-4 py-4">
                  <h2 className="line-clamp-2 break-words text-base font-bold leading-6 text-slate-900 [overflow-wrap:anywhere]">{task.title}</h2>
                  <p className="mt-1.5 line-clamp-2 min-h-10 break-words text-sm leading-5 text-slate-500 [overflow-wrap:anywhere]">{task.description || "Công việc không có mô tả."}</p>
