          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {archivedProjects.map((project: any) => (
              <article key={project._id} onClick={() => setSelectedDetail({ kind: "project", item: project })} className="flex min-h-[190px] cursor-pointer flex-col rounded-xl border border-slate-200 bg-white transition-colors hover:border-blue-300">
                ...
                <div className="min-w-0 flex-1 px-4 py-4"><h3 className="line-clamp-2 break-words text-base font-bold leading-6 text-slate-900 [overflow-wrap:anywhere]">{project.title}</h3><p className="mt-1.5 line-clamp-2 break-words text-sm leading-5 text-slate-500 [overflow-wrap:anywhere]">{project.description || "Dự án không có mô tả."}</p><p className="mt-4 flex min-w-0 items-center gap-2 text-xs text-slate-600"><Building2 className="size-3.5 shrink-0 text-slate-400" /><span className="truncate">{project.workspace?.name || "Không xác định workspace"}</span></p></div>
